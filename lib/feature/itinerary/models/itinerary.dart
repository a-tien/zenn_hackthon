...
class Itinerary {
  ...
  /// 生成用於部分更新的 JSON，只包含需要更新的字段且過濾掉 null 值
  Map<String, dynamic> toPartialUpdateJson() {
    final Map<String, dynamic> data = {};
    
    // 只添加有效值的字段
    if (name.isNotEmpty) data['name'] = name;
    data['useDateRange'] = useDateRange;
    data['days'] = days;
    data['startDate'] = startDate.toIso8601String();
    data['endDate'] = endDate.toIso8601String();
    
    if (destinations.isNotEmpty) {
      data['destinations'] = destinations.map((dest) => dest.toJson()).toList();
      data['destination'] = destination; // 保留舊格式兼容性
    }
    
    if (transportation.isNotEmpty) data['transportation'] = transportation;
    if (travelType.isNotEmpty) data['travelType'] = travelType;
    
    if (itineraryDays.isNotEmpty) {
      data['itineraryDays'] = itineraryDays.map((day) => day.toJson()).toList();
    }
  ...
  /// 生成只包含行程天數數據的更新 JSON
  Map<String, dynamic> toItineraryDaysUpdateJson() {
    return {
      'itineraryDays': itineraryDays.map((day) => day.toJson()).toList(),
  ...
  /// 生成用於 Firestore 部分更新的 JSON，使用 Timestamp 格式
  Map<String, dynamic> toPartialFirestoreUpdateJson() {
    final Map<String, dynamic> data = {};
